...
with tab1:
    ...
    search_query = st.text_input("🔍 Search player by name", placeholder="e.g. Salah")
    ...
    if players:
        import pandas as pd
        df = pd.DataFrame(players, columns=["Name", "Position", "Age", "Nationality", "Club"])
...
with tab4:
    ...
    if listings:
        ...
        df_tm["Transfer Value (€)"] = df_tm["Transfer Value (€)"].apply(lambda x: f"€{x:,}")
...
with tab5:
    ...
    with col_left:
        ...
        with st.form("add_player_form"):
            ...
            p_nationality = st.text_input("Nationality")
            ...
            if st.form_submit_button("Add Player"):
                if p_name.strip():
                    add_player(p_name, p_position, p_age, p_nationality, club_dict[club_choice])
